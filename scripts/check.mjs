...
assert.equal(denominatorEngine.generatedFrom.recipes,18);
...
for(const id of ['RH-hamina_site_amplification','RH-kemi_direct_value_chain_multiplier','RH-kemi_nebius_water_withdrawal_multiple','RH-GROUP-water_withdrawal_per_revenue','RH-GROUP-water_withdrawal_per_value_added']) assert(denominatorEngine.rabbitHoles.some(x=>x.id===id),`Missing denominator rabbit hole ${id}`);
