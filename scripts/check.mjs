...
for(const value of ['Symetrix Matrix v0.3','Taloudellinen volyymi / työpanos','Työllistävyys suhteessa volyymiin','73 P','27 P','74','61 P','96','63','Pre-op','Kontekstirivit','0,87 TWh','15,0 Mm³/v'])assert(sxPage.includes(value),`Displayed Symetrix mismatch: ${value}`);
const sxDetails=htmls.get(path.join(dist,'analyysit/datakeskukset/symetrix-mittarit/index.html')).$.text();
for(const value of ['Pisteytyksen perussääntö','0,386 M€/henkilötyövuosi','Metsä Fibre -liiketoiminnan proxy','PUE − 1','0,45 L/kWh','250 %','Candidate metrics','2 500','600 ha','1,5 M€','Evidenssin kattavuus','3 D + 2 Q','Työllisyyden systeemirajat E0–E3','4,2','8 000 Q','Scope jump','10×','17,6× Q','kaninkolo','Haminan sähkö','0,87 TWh/v','15 000 000 m³/v','1 300×'])assert(sxDetails.includes(value),`Displayed Symetrix details mismatch: ${value}`);
...
assert.equal(resourceScenarios.water_scope_jump.kemi.derived_design_withdrawal_m3_per_year,15000000);
assert.equal(resourceScenarios.water_scope_jump.descriptive_ratio.kemi_design_withdrawal_over_hamina_reported_withdrawal,1321);
