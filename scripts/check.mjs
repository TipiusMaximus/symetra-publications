...
for(const value of ['Symetrix Matrix v0.3','Taloudellinen volyymi / työpanos','Työllistävyys suhteessa volyymiin','73 P','27 P','74','61 P','100 E*','96','63','Pre-op','Kontekstirivit','0,87 TWh','15,0 Mm³/v','80 Mm³/v','0,0014 L/kWh IT','25,1 m³/M€','6 440 m³/M€','Sähkö / liikevaihto','1,92 GWh/M€','0,344 GWh/M€','12 623 m³/M€','35,4 m³/t','15,0 m³/t'])assert(sxPage.includes(value),`Displayed Symetrix mismatch: ${value}`);
const sxDetails=htmls.get(path.join(dist,'analyysit/datakeskukset/symetrix-mittarit/index.html')).$.text();
for(const value of ['Pisteytyksen perussääntö','0,386 M€/henkilötyövuosi','Metsä Fibre -liiketoiminnan proxy','PUE − 1','0,45 L/kWh','250 %','Candidate metrics','2 500','600 ha','1,5 M€','Evidenssin kattavuus','3 D + 2 Q','Työllisyyden systeemirajat E0–E3','4,2','8 000 Q','Scope jump','10×','17,6× Q','kaninkolo','Haminan sähkö','0,87 TWh/v','15 000 000 m³/v','1 300×','80 miljoonaa m³/v','0,0014 L/kWh IT','100 E*','Resurssivirran neljä ulottuvuutta','Vesi suhteessa liikevaihtoon','25,1 m³/M€','6 440 m³/M€','250×','257×','Denominator Engine v0.1','Sähkö suhteessa liikevaihtoon','1,92 GWh/M€','0,344 GWh/M€','533 kWh/t','416 M€','Nettoriippuvuus sähköverkosta','−150 %','S ~+100 %','32,819 Mm³/v','13,931 Mm³/v','35,4 m³/t','15,0 m³/t','503×'])assert(sxDetails.includes(value),`Displayed Symetrix details mismatch: ${value}`);
...
assert.equal(denominatorEngine.generatedFrom.recipes,12);
...
for(const id of ['RH-hamina_site_amplification','RH-kemi_direct_value_chain_multiplier','RH-GROUP-water_withdrawal_per_revenue']) assert(denominatorEngine.rabbitHoles.some(x=>x.id===id),`Missing denominator rabbit hole ${id}`);
