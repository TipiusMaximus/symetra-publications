...
for(const value of ['Symetrix Matrix v0.3','Taloudellinen volyymi / työpanos','Työllistävyys suhteessa volyymiin','73 P','27 P','74','61 P','96','63','Pre-op','Kontekstirivit','0,87 TWh','15,0 Mm³/v','80 Mm³/v','0,0014 L/kWh IT','25,1 m³/M€','6 440 m³/M€'])assert(sxPage.includes(value),`Displayed Symetrix mismatch: ${value}`);
const sxDetails=htmls.get(path.join(dist,'analyysit/datakeskukset/symetrix-mittarit/index.html')).$.text();
for(const value of ['Pisteytyksen perussääntö','0,386 M€/henkilötyövuosi','Metsä Fibre -liiketoiminnan proxy','PUE − 1','0,45 L/kWh','250 %','Candidate metrics','2 500','600 ha','1,5 M€','Evidenssin kattavuus','3 D + 2 Q','Työllisyyden systeemirajat E0–E3','4,2','8 000 Q','Scope jump','10×','17,6× Q','kaninkolo','Haminan sähkö','0,87 TWh/v','15 000 000 m³/v','1 300×','80 miljoonaa m³/v','0,0014 L/kWh IT','Resurssivirran neljä ulottuvuutta','Vesi suhteessa liikevaihtoon','25,1 m³/M€','6 440 m³/M€','250×'])assert(sxDetails.includes(value),`Displayed Symetrix details mismatch: ${value}`);
...
assert(evidenceJson.some(x=>x.id==='EV-KEMI-WATERREV-001'&&x.value===6442));
assert(evidenceJson.some(x=>x.id==='EV-WATERREV-GAP-001'&&Math.abs(x.value-256.8)<0.01));
...
assert(metricRegistry.metrics.some(x=>x.id==='non_seawater_water_intensity_proxy'&&x.readiness==='estimate_ready_not_scoreable'));
