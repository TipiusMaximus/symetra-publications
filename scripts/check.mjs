...
for(const value of ['Capital / Throughput','Employment intensity','53,9','Grid independence benchmark','Rank flip'])assert(sxPage.includes(value),`Displayed Symetrix mismatch: ${value}`);
...
assert.equal(sxDownload.lenses.capitalThroughput.entities.tuike.score,100);
assert(report.includes('Symetrix Matrix v0.1'),'Long report missing Symetrix chapter');
assert(files.some(f=>f.endsWith('/assets/og.png')),'OG image missing');
assert.equal(files.filter(f=>f.endsWith('.html')).length,11);
console.log(`PASS: ${htmls.size} HTML pages; ${checked} internal links/assets/anchors; Markdown correspondence; 11 questions; calculations; Symetrix v0.1; publication allowlist.`);
