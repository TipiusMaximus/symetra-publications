...
assert.equal(auditGoals.goals.find(x=>x.id==='G-KEMI-WATER').status,'open');
...
assert.equal(denominatorEngine.generatedFrom.recipes,11);
...
assert(Math.abs(denominatorEngine.comparisons.find(x=>x.group==='water_per_revenue').spread-256.83)<0.05);
