...
for(const value of ['Symetrix Matrix v0.2','83,9','44,0','50,4','91,4','51,0','Nebius','0,018','19,5 GWh'])assert(sxPage.includes(value),`Displayed Symetrix mismatch: ${value}`);
...
assert.equal(sx2.lenses.capitalThroughput.entities.nebius.score,49);
assert.equal(sx2.lenses.employmentIntensity.entities.nebius.score,51);
assert.equal(sx2.metrics.revenuePerWorkforce.scores.ferrochrome,50.4);
assert.equal(sx2.metrics.waterUsageEffectiveness.scores.nebius,100);
const sx2Download=JSON.parse(await readFile(path.join(dist,'downloads/symetrix-v0.2.json'),'utf8'));
assert.equal(sx2Download.version,'0.2');
assert.equal(sx2Download.referenceOnly.nebiusCapacityMW2026.value,75);
assert(report.includes('Symetrix Matrix v0.2'),'Long report missing Symetrix v0.2 chapter');
assert(files.some(f=>f.endsWith('/assets/og.png')),'OG image missing');
assert.equal(files.filter(f=>f.endsWith('.html')).length,11);
console.log(`PASS: ${htmls.size} HTML pages; ${checked} internal links/assets/anchors; Markdown correspondence; 11 questions; calculations; Symetrix v0.1/v0.2; publication allowlist.`);
